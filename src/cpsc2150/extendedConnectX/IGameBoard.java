 ...
 *              Win = user defined number of tokens (all X or all O) in a row horizontally, vertically, or diagonally    &&
 *              Win < row && win < column
...
// checks moves right
        ...
        while (counter < getNumToWin()) {
            ...
            if (!isPlayerAtPos(posOne, player)) {
                break;
            }
...
// check moves left
        ...
        while (counter < getNumToWin()) {
            ...
            if (!isPlayerAtPos(posTwo, player)) {
                break;
            }
            if (counter == getNumToWin()){
                return true;
...
// check counter for win
    ...
    default public boolean checkDiagWin(BoardPosition pos, char player){
        ...
        while (counter < getNumToWin()) {
            // exceed bounds
            if (r+offset >= getNumRows() || c+offset >= getNumColumns()){
                break;
            }
        ...
        while (counter < getNumToWin()) {
            // exceed bounds
            if (r+offset >= getNumRows() || c-offset < 0){
                break;
            }
            BoardPosition posTwo = new BoardPosition(r+offset, c-offset);
            if (isPlayerAtPos(posTwo, player) && counter != getNumToWin()) {
        ...
        while (counter < getNumToWin()) {
            // exceed bounds
            if (r-offset < 0 || c+offset >= getNumColumns()){
                break;
            }
            BoardPosition posTwo = new BoardPosition(r-offset, c+offset);
            if (isPlayerAtPos(posTwo, player) && counter != getNumToWin()) {
